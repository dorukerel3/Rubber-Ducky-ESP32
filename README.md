...
A custom **ESP32-S3** based hardware testing and **HID (Human Interface Device)** emulation tool, packaged in a footprint that closely resembles a standard USB flash drive. Developed as an independent hardware research project, the primary objective was to explore PCB design, microcontroller architecture, and rapid manufacturing processes end to end — from schematic capture through production-ready fabrication files. The board operates as a programmable USB device capable of emulating keyboard and mouse input, running a **DuckyScript-style payload interpreter** directly from a MicroSD card, with substantial onboard storage for complex scripts and payload sets.
...
On power-up it mounts a MicroSD card, scans for numbered `.txt` payload files, and executes them in order through an onboard interpreter that understands a DuckyScript-style command set. The design prioritizes two things: a compact, unobtrusive form factor that mimics an ordinary flash drive, and enough removable storage to hold and stage large, multi-part payloads. Every design and manufacturing artifact required to fabricate, assemble, and program the board independently is included in this repository.
...
- **DuckyScript-style payload interpreter** running on-device, supporting strings, key combinations, modifiers, function keys, navigation keys, mouse movement/clicks/scrolling, delays, and loops.
- **Multi-stage payload execution** — numbered `.txt` files on the SD card run sequentially, so complex operations can be split across ordered scripts.
...
- **High-capacity module option** — the `ESP32-S3-WROOM-2-N32R8V` variant (32 MB flash / 8 MB PSRAM) maximizes onboard capacity; the `N16R8` variant is a drop-in alternative.
...
| SD connector | Hirose DM3AT-SF-PEJM5 | microSD push-push | Removable payload storage |
| USB connector | Molex 48037-2200 | USB-A horizontal | Host connection and power |
| Status LED | LED | 0805 | Ready / run / error signaling |
| Passives | 1× 10 kΩ, 2× 22 Ω, 1× 330 Ω resistors; 2× 10 µF, 2× 100 nF capacitors | 0805 | USB data termination (22 Ω), pull-up, LED limiting, decoupling |
...
| Named keys | `ENTER`, `TAB`, `SPACE`, `ESC`/`ESCAPE`, `UP`, `DOWN`, `LEFT`, `RIGHT`, `CAPSLOCK`, `SCROLLLOCK`, `PAUSE`, `MENU`/`APP`, `F1`–`F24` |
| `MOUSE_MOVE <x> <y>` | Moves the cursor by a relative offset (auto-chunked to HID's ±127 range) |
...
- At runtime the files are collected and sorted **numerically**, then executed in ascending order, so multi-part operations can be split across ordered stages.
...
| SD mount failure | Rapid blink (~10 Hz) |
| No payloads found / read error | Slow blink (~1 Hz) |
...
- The two 22 Ω resistors are the USB D+/D− series terminations — do not omit them.
...
2. Select an **ESP32-S3** board, and under Tools enable **USB CDC / USB HID** (native USB) and set the flash size to match your module.
3. Compile and upload to the board.
4. Format a MicroSD card as FAT32 and place your numbered payloads (`1.txt`, `2.txt`, …) at its root.
